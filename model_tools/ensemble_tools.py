...
class ANIModel_charge_dist(torch.nn.ModuleList):
    ...
    def forward(self, species_coordinates, total_charge=0):
        ...
        return species, output, dipole, coulomb, excess_charge, output_c
...
class Ensemble(torch.nn.ModuleList):
    ...
    def forward(self, species_input: Tuple[Tensor, Tensor]) -> SpeciesEnergiesDipoleCharge:
        ...
        dip_ls=[]
        for x in self:
            species, energy, dipole, coulomb, excess_charge, charge = x((species_input))
            eng += energy
            dip += dipole
            col += coulomb
            charges += charge
            eng_ls.append(energy)
            dip_ls.append(torch.sqrt(torch.sum(dipole**2)).item())
        species, _ = species_input
        return SpeciesEnergiesDipoleCharge(species, eng / self.size, dip / self.size, col / self.size, charges / self.size, eng_ls, dip_ls)
